...
public sealed record GetAllDoctorQuery() : IRequest<Result<List<GetAllDoctorDto>>>;

internal sealed class GetAllDoctorQueryHandler(
    IDoctorRepository doctorRepository
) : IRequestHandler<GetAllDoctorQuery, Result<List<GetAllDoctorDto>>>
{
    public async Task<Result<List<GetAllDoctorDto>>> Handle(GetAllDoctorQuery request, CancellationToken cancellationToken)
    {
