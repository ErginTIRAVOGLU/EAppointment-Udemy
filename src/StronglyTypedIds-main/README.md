...
```csharp
// Simple definition - the library uses convention-based detection
// No inheritance needed! Just define a record struct with a Value property
...
```csharp
// Basit tanımlama - kütüphane convention-based algılama kullanır
// Inheritance'a gerek yok! Sadece Value property'si olan bir record struct tanımlayın
