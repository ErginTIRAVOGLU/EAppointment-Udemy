...
- **Conditional Compilation**: Uses `#if NET9_0_OR_GREATER` for version-specific Guid generation
- **Usage Examples**:
...
All converters/filters use reflection to detect strongly typed IDs. **This pattern is CRITICAL** - it appears identically in:
- `StronglyTypedIdConvention` (EF Core)
- `StronglyTypedIdJsonConverterFactory` (JSON serialization)
- `StronglyTypedIdSchemaFilter` (OpenAPI/Swagger)

```csharp
// Detection pattern used throughout (must be consistent):
...
**When adding new value types**, update all three `SupportedValueTypes` collections AND all switch statements for serialization/schema mapping.

### 3. EF Core Integration (`EFCore/`)
- **Convention-based**: `ConfigureStronglyTypedIds()` extension on `ModelConfigurationBuilder`
- **Implementation**: Uses `IModelFinalizingConvention` to scan all entity properties after model configuration
...
- **Converter pattern**: Uses reflection in constructor to extract/inject Value property via `Activator.CreateInstance`
...
- **Important**: JSON converter is registered globally via `Configure<JsonOptions>`, not per-controller
...
- **Dependency Model**: All PackageReferences use `PrivateAssets="All"` to avoid imposing versions on consumers
...
- **.NET version awareness**: Always use `#if NET9_0_OR_GREATER` when adding features that depend on .NET 9+ APIs
...
- **Don't** modify the detection pattern in one place without updating all three integration points (EF Core, JSON, Swagger)
